...
function getExtendedRelationships(
  ...
  const getParents = (pid: string) =>
    relationships.filter(r => r.relationship_type === "parent" && r.to_person_id === pid).map(r => r.from_person_id);
  const getChildren = (pid: string) =>
    relationships.filter(r => r.relationship_type === "parent" && r.from_person_id === pid).map(r => r.to_person_id);
  ...
  for (const parentId of parents) {
    for (const gpId of getParents(parentId)) {
  ...
  for (const gpId of grandparentIds) {
    for (const ggpId of getParents(gpId)) {
  ...
  for (const childId of children) {
    for (const gcId of getChildren(childId)) {
  ...
  for (const parentId of parents) {
    for (const gpId of getParents(parentId)) {
      for (const auId of getChildren(gpId)) {
  ...
  // Nieces/Nephews
  for (const sid of siblingIds) {
    for (const nnId of getChildren(sid)) {
  ...
  for (const auId of auntUncleIds) {
    for (const cId of getChildren(auId)) {
